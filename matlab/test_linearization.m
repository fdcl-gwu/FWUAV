...
for k=1:N
    ...
    [JJ_new, EL_rhs_new] = EL_equation_terms(INSECT, x(:, k), R(:,:,k)*expmhat(eta), Q_R(:,:,k), Q_L(:,:,k), Q_A(:,:,k), x_dot(:, k), W(:, k), W_R(:, k), W_L(:, k), W_A(:, k), x_ddot(:, k), W_dot(:, k), W_R_dot(:, k), W_L_dot(:, k), W_A_dot(:, k), F_R(:, k), F_L(:, k), M_R(:, k), M_L(:, k), tau(:, k));
...
for k=1:N
    ...
    [J_new, ~] = inertia(INSECT, R(:,:,k), Q_R(:,:,k), Q_L(:,:,k), Q_A(:,:,k)*expmhat(eta), x_dot(:,k), W(:,k), W_R(:,k), W_L(:,k), W_A(:,k));
    del_J = J_new - J;
...
for k=1:N
    ...
    [~, K_new] = inertia(INSECT, R(:,:,k)*expmhat(eta), Q_R(:,:,k), Q_L(:,:,k), Q_A(:,:,k), x_dot(:,k), W(:,k), W_R(:,k), W_L(:,k), W_A(:,k));
    del_K = K_new - K;
...
for k=1:N
    ...
    [~, K_i] = inertia_wing_sub(INSECT.m_R, INSECT.mu_R, INSECT.nu_R, INSECT.J_R, R(:,:,k), Q_R(:,:,k), x_dot(:,k), W(:,k), W_R(:,k));
    [~, K_i_new] = inertia_wing_sub(INSECT.m_R, INSECT.mu_R, INSECT.nu_R, INSECT.J_R, R(:,:,k), Q_R(:,:,k)*expmhat(eta), x_dot(:,k), W(:,k), W_R(:,k));
    del_K_i = K_i_new - K_i;
    lhs = del_K_i* [x_dot(:,k);W(:,k);W_R(:,k);];
