...
des.df_a_1_by_dtheta_0 = -0.2e-3 / 0.1; % dtheta_0 > 0, + component dec, - comp inc in magnitude
des.df_a_3_by_dtheta_0 = -0.3e-3 / 0.1; % dtheta_0 > 0
...
x0 = des.x0 + dx0;
x0 = 0.1*[-0.0052; 0; 0.0144;]; % Unstable IC?
% x0 = [0; 0; 0.0144;];
x_dot0 = des.x_dot0 + dx_dot0;
X0 = [x0; x_dot0;];
N = 10001; % N = 2001 causes some numerical divergence
N_period = 100;
...
% 
% temp_A = [sign(f_a_im1(1)) * des.df_a_1_by_dphi_m, 0, des.df_a_1_by_dtheta_0, sign(f_a_im1(4)) * des.df_a_1_by_dtheta_A_m;
%           0, des.df_a_2_by_dphi_m, 0, 0;
%           des.df_a_3_by_dphi_m, 0, des.df_a_3_by_dtheta_0, sign(f_a_im1(6)) * des.df_a_3_by_dtheta_A_m;
%           wt * sign(f_a_im1(1)) * des.df_a_1_by_dphi_m, 0, 0, -(1-wt) * sign(f_a_im1(4)) * des.df_a_1_by_dtheta_A_m];
% mul_phi = R * Q_R * (sign(F_R) .* [des.df_r_1_by_dphi_m; 0; des.df_r_3_by_dphi_m]);
% temp_A = [mul_phi(1), 0, des.df_a_1_by_dtheta_0, sign(f_abd(1)) * des.df_a_1_by_dtheta_A_m;
%           0, des.df_a_2_by_dphi_m, 0, 0;
%           mul_phi(3), 0, sign(f_a(3))*des.df_a_3_by_dtheta_0, sign(f_abd(3)) * des.df_a_3_by_dtheta_A_m;
%           wt * mul_phi(1), 0, 0, -(1-wt) * sign(f_abd(1)) * des.df_a_1_by_dtheta_A_m];
e2 = [0; 1; 0];
mul_phi = R * Q_R * [get_m(sign(F_R(1)), des.df_r_1_by_dphi_m); 0; get_m(sign(F_R(3)), des.df_r_3_by_dphi_m)];
mul_theta = 2 * R * Q_R * hat(e2) * F_R;
temp_A = [mul_phi(1), 0, mul_theta(1), sign(f_abd(1)) * des.df_a_1_by_dtheta_A_m;
          0, des.df_a_2_by_dphi_m, 0, 0;
          mul_phi(3), 0, mul_theta(3), sign(f_abd(3)) * des.df_a_3_by_dtheta_A_m;
          wt * mul_phi(1), 0, 0, -(1-wt) * sign(f_abd(1)) * des.df_a_1_by_dtheta_A_m];
% f_signs = sign(f_a_im1(1:3));
rhs = [pos_err; 0];
...
bound = 0.5;
% dang(abs(dang) > bound) = bound * sign(dang(abs(dang) > bound)); % constrain the control
dphi_m_R = dang(1) + dang(2);
dphi_m_L = dang(1) - dang(2);
dtheta_0 = dang(3);
dtheta_A_m = dang(4);
dpsi_m = 0;
