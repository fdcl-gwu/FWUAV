...
des=load('sim_QS_x_hover.mat',...
    'INSECT', 't', 'N', 'x', 'x_dot', 'f_tau', 'x0', 'x_dot0', 'WK');
...
x0 = des.x0 + dx0;
x0 = [-0.0052; 0; 0.0144;]; % Unstable IC?
% x0 = [0; -0.01; 0;];
...
mul_phi = [des.df_a_1_by_dphi_m(round((3 - sign(des.f_a(1, i)))/2)); 0; des.df_a_3_by_dphi_m];
% e2 = [0; 1; 0];
% mul_theta = 2 * R * Q_R * hat(e2) * F_R;
mul_theta = [des.df_a_1_by_dtheta_0(round((3 - sign(des.f_a(1, i)))/2)); 0; des.df_a_3_by_dtheta_0];
mul_theta_A = [des.df_a_1_by_dtheta_A_m(round((3 - sign(des.f_abd(1, i)))/2)); 0; ...
    des.df_a_3_by_dtheta_A_m(round((3 - sign(des.f_abd(3, i)))/2))];
