...
% [WK_arr, fval, exitflag, output] = fmincon(@(WK_arr) objective_func(WK_arr, WK, INSECT, N, x0, nonlcon),...
...
problem = createOptimProblem('fmincon','objective',@(WK_arr) objective_func(WK_arr, WK, INSECT, N, x0, nonlcon),...
...
%     'UseParallel',true,'InitialPopulationMatrix',ptmatrix,'MaxTime',12*3600,...
%     'HybridFcn',{@fmincon,hybridoptions});
% [WK_arr, fval, exitflag, output] = ga(@(WK_arr) objective_func(WK_arr, WK, INSECT, N, x0, nonlcon),...
%     length(WK_arr0), A, b, Aeq, beq, lb, ub, nonlcon, options);
%
% options = optimoptions('particleswarm','PlotFcn',@pswplotbestf,...
%     'UseParallel',true,'InitialSwarmMatrix',ptmatrix,'MaxTime',12*3600,...
%     'HybridFcn',{@fmincon,hybridoptions});
% [WK_arr, fval, exitflag, output] = particleswarm(@(WK_arr) objective_func(WK_arr, WK, INSECT, N, x0, nonlcon),...
...
function [J] = objective_func(WK_arr, WK, INSECT, N, x0, nonlcon)
%% Objective function
[WK, x_dot0] = get_WK(WK, WK_arr);
X0=[x0; x_dot0];
