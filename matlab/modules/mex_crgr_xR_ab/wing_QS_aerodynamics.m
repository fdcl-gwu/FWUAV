...
%     [F_rot_L M_rot_L alpha_L U_alpha_L_dot]=compute_rotational_force(INSECT, U_L, U_L_dot, e2);
    ...
    U_R = bsxfun(@plus, Ime2*Q_R'*(RTx_dot+hat_W*INSECT.mu_R) , bsxfun(@times, rs , (hat(Q_R*W+W_R)*e2)));
    U_L = bsxfun(@plus, Ime2*Q_L'*(RTx_dot+hat_W*INSECT.mu_L) , -bsxfun(@times, rs , (hat(Q_L*W+W_L)*e2)));
    c=polyval(INSECT.cr_poly,rs/INSECT.scale)*INSECT.scale;
    [L, D, M]=compute_LD(INSECT, U_R, hat_e2);
    L_R = sum(bsxfun(@times, L , c), 2)/INSECT.S * dr;
    D_R = sum(bsxfun(@times, D , c), 2)/INSECT.S * dr;
    M_R = sum(bsxfun(@times, M , bsxfun(@times, rs , c)), 2) /(INSECT.r_cp * INSECT.S) * dr;
    [L, D, M]=compute_LD(INSECT, U_L, hat_e2);
    L_L = sum(bsxfun(@times, L , c), 2)/INSECT.S * dr;
    D_L = sum(bsxfun(@times, D , c), 2)/INSECT.S * dr;
    M_L = sum(-bsxfun(@times, M , bsxfun(@times, rs , c)), 2) /(INSECT.r_cp * INSECT.S) * dr;
...
L = 0.5 * INSECT.rho * INSECT.S * (bsxfun(@times, C_L , bsxfun(@times, sign(bsxfun(@times, U(1, :) , U(3, :))) , bsxfun(@times, (hat_e2*U) , norm_U))));
D = - 0.5 * INSECT.rho * INSECT.S * (bsxfun(@times, C_D , bsxfun(@times, norm_U , U)));
...
idx = (norm_U ~= 0);
u = bsxfun(@rdivide, U(:, idx) , norm_U(idx));
