...
for iter=1:N_dagger_iters
	...
	for i=1:N_sims*N_scale
		...
		for period=1:N_periods
			...
			input = (1 ./ Weights.PerturbVariables)' .* get_error(X0, X_ref0);
            input_prev = (1 ./ Weights.PerturbVariables)' .* get_error(X0Prev, X_ref0);

            [nn_inp1, nn_inp2] = dnnTrain.norm_data(input_prev, input, zeros(60,1), ...
                inputs_mean, inputs_scale, targets_mean, targets_scale);
            nn_pred = dnnTrain.predictnet(dlnet, netParams, nn_inp1, nn_inp2);
            param = double(extractdata(nn_pred .* targets_scale + targets_mean));
            X0Prev = X0;
	...
	parfor i=1:N_pi
        ...
        for j=1:N_period_pi
		    inputs_pi(:, i, j) = (1 ./ Weights.PerturbVariables)' .* get_error(X(1+(j-1)*N_single, :)', X_ref0);
        end
	end
	time_iter = toc(titer);
    ...
    inputs_pi1 = reshape(inputs_pi(:, :, 1:(end-1)), size(inputs_pi, 1), []);
    inputs_pi2 = reshape(inputs_pi(:, :, 2:end), size(inputs_pi, 1), []);
    targets_pi = reshape(targets_pi(:, :, 2:end), size(targets_pi, 1), []);

    [inputs_pi1, inputs_pi2, targets_pi] = dnnTrain.norm_data(inputs_pi1, inputs_pi2, targets_pi, ...
        inputs_mean, inputs_scale, targets_mean, targets_scale);
	inputs_multiple{1} = [inputs_multiple{1}, extractdata(inputs_pi1)];
    inputs_multiple{2} = [inputs_multiple{2}, extractdata(inputs_pi2)];
	targets = [targets, extractdata(targets_pi)];
