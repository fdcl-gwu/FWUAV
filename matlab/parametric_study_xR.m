...
M_a_m = zeros(N_study, 3, N_params);
...
%%
    ...
    WK_R.phi_m = WK_R.phi_m + eps(i);
    WK_L.phi_m = WK_L.phi_m - eps(i);
    [f_aero(4, :, :), M_aero(2, :, :)] = aerodynamic_param(INSECT, WK_R, WK_L, t, X0, N, R, F_R);
    %
    WK_R = WK;  WK_L = WK;
    WK_R.theta_0 = WK_R.theta_0 + eps(i);
    WK_L.theta_0 = WK_L.theta_0 - eps(i);
    [~, M_aero(3, :, :)] = aerodynamic_param(INSECT, WK_R, WK_L, t, X0, N, R, F_R);
    %
    WK_R = WK;  WK_L = WK;
    WK_R.psi_m = WK_R.psi_m + eps(i);
    WK_L.psi_m = WK_L.psi_m - eps(i);
    [~, M_aero(4, :, :)] = aerodynamic_param(INSECT, WK_R, WK_L, t, X0, N, R, F_R);
    %
    WK_R = WK;  WK_L = WK;
    WK_R.theta_A_m = WK_R.theta_A_m + eps(i);
    WK_L.theta_A_m = WK_L.theta_A_m + eps(i);
    [~, M_aero(5, :, :), f_aero(5, :, :)] = aerodynamic_param(INSECT, WK_R, WK_L, t, X0, N, R, F_R);
end
...
%%
    ...
    for k=1:N    
        [~, R, ~, ~, Q_A, ~, W, ~, ~, ~, ~, W_A, W_A_dot, ~, ~, ~, ~, f_a, ~, f_tau]= eom_QS_xR(INSECT, WK_R, WK_L, t(k), X(k,:)');
